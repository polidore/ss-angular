...
This will be handled with the promise API in angular ($q).  You just assign a $scope object to an rpc call, and the value will be assigned when the rpc returns.  You don't use a callback.  For example:
...
Here we're assigning $scope.status to the return value of the services we've defined. As you can see, you can pass params or not, but you can't define a callback.  If you really need a callback, just call socketstream directly!
