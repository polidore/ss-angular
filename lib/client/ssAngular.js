...
module.factory('model', ['$rootScope', function($rootScope) {
  Object.getPrototypeOf($rootScope).unlinkModel = function(name, params, scopeName) {
    var scope = this;
  ...
  Object.getPrototypeOf($rootScope).linkModel = function(name, params, scopeName) {
    ...
    if(scope[scopeName]) {
      return; //already subscribed. sometimes angular likes to call controllers many times in a row
    }
    ...
    scope.$on('$destroy', function(s) {
      if(scope[scopeName]) {
        scope.unlinkModel(name, params, scopeName);
      }
    });
  };
}]);
