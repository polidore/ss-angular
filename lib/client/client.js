...
function parseRequest() {
  ...
  r.requestName = modelName + "/" + paramString;
  return r;
}
...
module.exports = function(responderId, config, send) {
  ...
  ss.registerApi('unlinkModel', function() {
    var req = parseRequest(arguments);
    if(!callbacks[req.requestName]) {
      throw new Error("Not subsctibed. Cannot unlink");
    }
    else {
      console.log("Ending model sync: " + req.requestName);
      var request = {
        m: "UNLINK",
        n: req.modelName,
        p: req.params
  ...
  ss.message.on(responderId, function(msg) {
    ...
    if(res.r === "OK") {
      if(res.o && res.n) {
        ...
        if(res.m === "UNLINK") {
          if(!callbacks[res.n]) {
            throw Error("Unknown model id for unsubscribe: " + res.n);
          }
          else {
        ...
        throw new Error("Unknown response: " + JSON.stringift(res));
      }
    }
    else {
