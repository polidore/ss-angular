      ...
      return {
        websocket: function(msg, meta, send) {
          ...
          var handleError = function(e) {
            ...
            ss.log.debug("<-".red, "SS-ANGULAR ERROR: ",msgLogName, req.method, req.requestName, e);
            if(e.stack) {
              ss.log.debug(e.stack.split("\n").splice(1).join("\n"));
            }
            ...
            try {
              ...
              var updateModel = function() {
                var timeoutId = setTimeout(function() {
                  ss.log.debug(msgLogName,"Model never called back after ".red + timeout/1000 + " seconds".red);
                },timeout);
                ...
                model(req, function(err, modelObj) {
                  ...
                  if(err) {
                    ss.log.debug("Received error from model. Stopping updates. Error: ",err);
                    try {
                      stopPolling(req.socketId,req.requestName);
                    }
                    catch(e) {
                      ss.log.debug('Error and could not stop polling'.red, e);
                    }
                    handleError(err);
                  }
                  else if(!client[req.requestName]) {
                    ss.log.debug(msgLogName, "Request removed");
                    return;
                  }
                  else {
                    var responseObj;
                    if(!modelObj) {
                      var error = 'NOAUTH';
                      ss.log.debug(msgLogName, error.red);
                      responseObj = {
                        r: error,
                        o: modelObj,
                        n: req.requestName
