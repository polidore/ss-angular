        ...
        websocket: function(msg, meta, send) {
          ...
          if(req.method === 'LINK') {
            try {
              ...
              client[req.requestName].intervalId = setInterval(function() {
                if(socket && socket.disconnected) {
                  ss.log(msgLogName,'Client Disconnected. Stopping updates'.yellow);
                  stopPolling(req.socketId, req.requestName);
                  return;
                }
                if(!client[req.requestName]) {
                  //already unsubscribed, but interval callback was in JS event queue.
                  ss.log("Ignoring callback after unsubscribe".red,msgLogName);
                  return;
                }
                updateModel();
              }, pollFreq);
...
var stopPolling = function(clientId,model) {
  ...
  var intervalId = client[model].intervalId;
  if(!intervalId) {
