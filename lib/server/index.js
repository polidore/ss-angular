...
module.exports = function(responderId, config, ss) {
  ...
  return {
    name: name,
    interfaces: function interfaces(middleware) {
      var model = require('./model')(ss,middleware);

      return {
        websocket: function websocket(msg, meta, send) {
          msg = JSON.parse(msg);
          ...
          var handleError = function handleError(e) {
            ...
            ss.log.error("<-".red, "SS-ANGULAR ERROR: ",msgLogName, req.method, req.requestName, e && e.toString());
            if(e.stack) {
          ...
          if(req.method === 'LINK') {
            try {
              ...
              var updateModel = function updateModel() {
                var timeoutId = setTimeout(function modelTimerCb() {
                  ss.log.warn(msgLogName,"Model never called back after ".red + timeout/1000 + " seconds".red);
                },timeout);

                var modelStart = Date.now();
                var request = client[req.requestName];
                if (request && request.lastHash)
                  req.lastHash = request.lastHash;

                model(req, function modelCb(err, modelObj) {
                  clearTimeout(timeoutId);

                  if(err) {
                    ss.log.error("Received error from model. Stopping updates. Error: ",err);
                    try {
                      stopPolling(req.socketId,req.requestName);
                    }
                    catch(e) {
                      ss.log.error('Error and could not stop polling'.red, e);
                    }
                    handleError(err);
                  }
                  else if(!client[req.requestName]) {
                    ss.log.warn(msgLogName, "Request removed");
                    return;
                  }
                  else {
                    var responseObj;
                    if(!modelObj) {
                      var error = 'NOAUTH';
                      ss.log.warn(msgLogName, error.red);
                      responseObj = {
                        r: error,
                        o: modelObj,
                        n: req.requestName
                    ...
                    else if(modelObj.hash && request && request.lastHash === modelObj.hash) {
              ...
              var poll = function poll(request) {
                if(!request) {
                  return;
                }

                request.timeoutId = setTimeout(function pollDelay() {
                  if(!socket || socket.readyState === 'closed') {
                    ss.log.info(msgLogName,'Client Disconnected. Stopping updates'.yellow);
                    try {
                      stopPolling(req.socketId, req.requestName);
                    }
                    catch (e) {
                      ss.log.error('Stop polling failed, probably a timer collision: ' +  e.toString());
                    }
                    return;
                  }
                  try {
              ...
              ss.log.debug("<-".green,res.r.green,msgLogName,res.i);
              ...
              try {
                ...
                ss.log.debug(msgLogName, "Could not stop polling in an error case. Not always a problem: ", e);
              }
              handleError(e);
            }
          }
            ...
            try {
              ...
              ss.log.debug("Could not unsubscribe from ".red, JSON.stringify(req));
              handleError(e);
              return;
            }
            var res = {};
...
var stopPolling = function stopPolling(clientId,modelId) {
  if(!clients) {
